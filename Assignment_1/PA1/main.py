...
# object is used to run six tests of search functions. These tests involve
# three algorithms, with each tested with and without repeated state
# checking. The three algorithms are uniform cost search, greedy best-first
# search, and A* search. When appropriate, search algorithms are passed
# a heuristic evaluation function. Solutions are output to the console,
# along with search algorithm performance statistics.
#
# David Noelle - Tue Sep 26 13:12:28 PDT 2023
#
...
def main():
    print("HEURISTIC SEARCH ALGORITHM COMPARISON")
    problem = RouteProblem(test_map, start_location, goal_location)
    h = HeuristicFunction(problem)



    print("TESTING UNIFORM COST SEARCH WITHOUT REPEATED STATE CHECKING")
    vars.node_expansion_count = 0
    sol = ucost.uniform_cost_search(problem, False)
    print("Solution:")
    if sol is None:
        ...
        print("Path Cost =", '%.2f' % sol.path_cost)
        print("Number of Node Expansions =", vars.node_expansion_count)

    print("TESTING UNIFORM COST SEARCH WITH REPEATED STATE CHECKING")
    vars.node_expansion_count = 0
    sol = ucost.uniform_cost_search(problem, True)
    print("Solution:")
    if sol is None:
        print("No solution found.")
    else:
    ...
    print("TESTING GREEDY BEST-FIRST SEARCH WITHOUT REPEATED STATE CHECKING")
    vars.node_expansion_count = 0
    sol = greedy.greedy_search(problem, h, False)
    print("Solution:")
    if sol is None:
        print("No solution found.")
    else:
    ...
    print("TESTING GREEDY BEST-FIRST SEARCH WITH REPEATED STATE CHECKING")
    vars.node_expansion_count = 0
    sol = greedy.greedy_search(problem, h, True)
    print("Solution:")
    if sol is None:
        print("No solution found.")
    else:
        ...
        print("Path Cost =", '%.2f' % sol.path_cost)
        print("Number of Node Expansions =", vars.node_expansion_count)
    print("TESTING A-STAR SEARCH WITHOUT REPEATED STATE CHECKING")
    vars.node_expansion_count = 0
    sol = astar.a_star_search(problem, h, False)
    print("Solution:")
    if sol is None:
        print("No solution found.")
    else:
        ...
        print("Path Cost =", '%.2f' % sol.path_cost)
        print("Number of Node Expansions =", vars.node_expansion_count)
    print("TESTING A-STAR SEARCH WITH REPEATED STATE CHECKING")
    vars.node_expansion_count = 0
    sol = astar.a_star_search(problem, h, True)
    print("Solution:")
    if sol is None:
        print("No solution found.")
    else:
